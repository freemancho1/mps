...
[역할]
  ...
  - 봉 t에서 신호가 났다고 가정하고, "다음 봉(t+1)의 시가"에 진입한 뒤
    time_horizon분 동안:
      · 익절선(+take_profit) 먼저 도달 → BUY (매수했어야 할 자리)
      · 손절선(-stop_loss) 먼저 도달 → HOLD (매수 부적합)
      · 시간 만료 → HOLD (방향 불명확)

[보수성]
  - 같은 봉에서 익절선·손절선을 동시에 포함하면 어느 쪽이 먼전인지 알 수 없음
    → StopLossTakeProfitGuard에서 이 경우 "손절 우선"으로 처리

[클래스 인덱스]
  - CrossEntropyLoss 용 정수 매핑: cfg.data.dir2idx(BUY=0, HOLD=1)
"""
...
class TripleBarrierLabeler:
    def __init__(
        ...
        self._take_profit: float = cfg.trade.barrier.take_profit \
            if take_profit is None else take_profit 
        self._stop_loss: float = cfg.trade.barrier.stop_loss \
            if stop_loss is None else stop_loss 
        self.time_horizon: int = cfg.trade.barrier.time_horizon \
    ...
    def label(self, bars: list[Bar]) -> np.ndarray:
        ...
        반환: shape [len(bars)] int64
              마지막 봉(다음 봉 없음)과 horizon 미확정 구간은 HOLD 
              → 학습 시 Dataset에서 마지막 horizon개 시점을 제외해 라벨 누수 방지.
        """
        bar_count = len(bars)

        open_arr = np.fromiter(
            (bar.open for bar in bars), dtype=np.float64, count=bar_count)
        high_arr = np.fromiter(
            (bar.high for bar in bars), dtype=np.float64, count=bar_count)
        low_arr = np.fromiter(
            (bar.low for bar in bars), dtype=np.float64, count=bar_count)
        ...
        # k=진입 봉(t+1) 기준 오프셋. k=0이 진입 봉 자신.
        for k in range(self.time_horizon):
            n_valid = bar_count - 1 - k     # bar_count-1인 t의 수 → t+1+k
            if n_valid <= 0:
                break 

            high = high_arr[1+k:1+k+n_valid]
            low = low_arr[1+k:1+k+n_valid]
        ...
        logger.debug(msg.pp.features.label_size(bars, label_arr))
        return label_arr
    
    def label_distribution(self, labels: np.ndarray) -> dict[SignalDirection, int]:
        """ 라벤 분포 확인 (클래스 불균형 진단용) """
        return {
