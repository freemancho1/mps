...
class TripleBarrierDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        bars: list[Bar],
        track: str = cfg.model.default_track,   # numeric
        lookback: Optional[int] = None,
        labeler: Optional[TripleBarrierLabeler] = None,
    ) -> None:
        assert track in (cfg.model.numeric_track, cfg.model.pattern_track)
        self._track = track 
        ...
        # 유효 구간: 윈도우 충족(pit >= lookback-1) and 라벨 확정(pit < bar_count - horizon)
        start_pit = self._lookback - 1
        end_pit = bar_count - horizon
        logger.debug(msg.pp.features.window_range(start_pit, end_pit))

        if track == cfg.model.numeric_track:
        ...
        self._X = X 
        self._y = (
            labels[start_pit:end_pit].astype(np.int64)
    ...
    def _build_pattern_window(self, bars: list[Bar], start: int, end: int) -> np.ndarray:
        ...
        for pit in range(start, end):
            ...
            prices = window[:, :4]
            prices_min, prices_max = prices.min(), prices.max()
    ...
    def class_counts(self) -> np.ndarray:
        ...
        return np.bincount(self._y, minlength=cfg.train.lstm_settings.num_classes)
