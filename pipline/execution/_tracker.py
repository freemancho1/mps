...
[역할]
  - 현재 열려 있는 주문(미체결 또는 체결 후 포지션 보유 중)을 추적함.
...
class OrderStateTracker:
    def __init__(self) -> None:
        ...
        self._results: dict[str, OrderResult] = {}  # key: order_id
        
    def register(self, order: Order) -> None:
        """ 새 주문 등록. order_id가 없는 주문은 무시. """
        if order.order_id:
    ...
    def get_result(self, order_id: str) -> Optional[OrderResult]:
        """ 주문 ID로 결과 조회 """
    ...
    def open_orders(self) -> list[Order]:
        ...
        - 등록된 주문 중 결과가 없거나 FILLED가 아닌 주문을 반환.
          (실거래 시) 장 마감 강제청산 대상 포지션 식별에 사용.
        """
        closed_ids = {
            order_id for order_id, result in self._results.items()
                if result.status == "FILLED"
        }
        return [
            order for order_id, order in self._orders.items() 
                if order_id not in closed_ids
        ]