...
def _configure_matplotlib():
    """ 실행 환경에 맞춰 Matplotlib의 한글 폰트 및 기본 스타일 설정 """
    system_os = platform.system()
    _config = _Config()
    _font_paths = _config.font_paths.get(system_os, [])
    
    font_path = next((
        path for path in _font_paths if os.path.exists(path)
    ), None)
    if not font_path:
    ...
    global MPF_STYLE
    MPF_STYLE = mpf.make_mpf_style(base_mpf_style="charles", rc=rc)
