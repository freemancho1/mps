...
[신호 임계 기준치는 별개]
  - min_profitable_return()은 '이 거래가 할만한가?'를 판단하는 기준이므로,
    슬리피지까지 포함한 총 왕복 경제비용에 대한 비율(≒0.41%)을 리턴함.
  - 이 함수는 현재 사용하지 않음
"""
...
class CostModel:
    def __init__(self) -> None:
        self._buy_rate = cfg.run.commission_rate
        self._sell_rate = cfg.run.commission_rate + cfg.run.tax_rate
        # 왕복 비용에는 슬리피지 비율이 포함되어 있음.
        self._roundtrip_rate_with_slippage = cfg.run.roundtrip_cost_rate
