...
[관측 가능성 원칙]
  - 전략이 왜 진입·청산했는지, 어떤 피처가 신호를 만들었는지,
    나중에 분석할 수 있어야 함.
  - 모든 신호와 주문을 JSONL(JSON Line) 형식으로 저장
...
class SignalLogger:
    def __init__(
        ...
        self._log_dir = log_dir or cfg.log.dir
        self._log_fname = log_fname or cfg.log.signal_log_fname
        self._path = self._log_dir / self._log_fname 
        self._log = logging.getLogger(cfg.log.title_signal)

    def log(self, signal: NumericSignal|PatternSignal|TradeSignal) -> None:
...
class OrderLogger:
    def __init__(
        ...
        self._log_dir = log_dir or cfg.log.dir
        self._log_fname = log_fname or cfg.log.order_log_fname
        self._path = self._log_dir / self._log_fname
        self._log = logging.getLogger(cfg.log.title_order)
